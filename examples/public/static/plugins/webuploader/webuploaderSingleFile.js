...
(function (window_, $) {
    ...
    uploaderController.prototype.setCurrentFileObject = function (file) {
...
// console.log(file.id, previous + " -> " + current);
                ...
                if (current === "complete")
                {
                    ...
                    if (this_.chunkStatus[hash])
                    {
                        if ((this_.chunkStatus[hash]["is_merge"] === 1))
                        {
                            ...
                            const eventSource = new EventSource(this_.mergeApi + "?hash=" + hash);
            ...
            if (this_.currentFile)
            {
                uploader.removeFile(this_.currentFile);
            }

            this_.setCurrentFileObject(file);
            this_.addFile(file);
